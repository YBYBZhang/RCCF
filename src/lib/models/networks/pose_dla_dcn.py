...
class RNNEncoder(nn.Module):
  ...
  def forward(self, input_labels):
    ...
    if self.variable_lengths:
      ...
      #print("input length list: ", input_lengths_list)
      #print("label size 1: ", input_labels.size(1))
...
class DLARef(nn.Module):
    ...
    def forward(self, x, sentence):
        ...
        center_map = self.sigmoid(center_logit)
...
def get_ref_net(num_layers, heads, vocab_size, word_embedding_size=1024, word_vec_size=512, hidden_size=512, head_conv=256, down_ratio=4):
